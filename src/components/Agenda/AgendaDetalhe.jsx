...
export default function AgendaDetalhe({ reserva, onClose, onDelete }) {
  ...
  const access = String(
    user?.nivel_acesso || user?.role || user?.perfil || ""
  )
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
  ...
  return (
    <div className="agenda-detalhe-overlay">
      <div className="agenda-detalhe-modal">
        <button className="agenda-detalhe-close" onClick={onClose} title="Fechar">
          <FaTimes size={22} />
        </button>
        ...
        <div className="agenda-detalhe-info">
          ...
          <p><strong>Duração:</strong> {reserva?.duracao ?? "-"} min</p>
        </div>
