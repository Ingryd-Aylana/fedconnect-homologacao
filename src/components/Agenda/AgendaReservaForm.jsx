...
export default function AgendaReservaForm({
  ...
  const [participantes, setParticipantes] = useState(initialData.participantes?.join(", ") || "");
  ...
  function handleSubmit(e) {
    ...
    if (!participantes.trim()) return setErro("Informe ao menos um participante.");
  ...
  return (
    <form className="agenda-form" onSubmit={handleSubmit}>
      ...
      <div className="agenda-form-group">
        <label>Participantes <span className="agenda-form-hint">(Separe por vírgula)</span>:</label>
        <input
          className="agenda-form-input"
          type="text"
          value={participantes}
